...
export default function App() {
  ...
  const [showImportWizard, setShowImportWizard] = useState<boolean>(() => {
    const dismissed = loadStorage<boolean>('w2_import_dismissed', false);
    return !dismissed;
  });

  const handleImportDismiss = () => {
    saveStorage('w2_import_dismissed', true);
    setShowImportWizard(false);
  };

  const handleImportDone = (result: { bookmarks: number; history: number; passwords: number }) => {
    saveStorage('w2_import_dismissed', true);
    setShowImportWizard(false);
    setBookmarks(getBookmarks());
    setHistory(getHistory());
    const parts: string[] = [];
    if (result.bookmarks) parts.push(`${result.bookmarks} закладок`);
    if (result.history)   parts.push(`${result.history} записів історії`);
    if (result.passwords) parts.push(`${result.passwords} паролів`);
    if (parts.length) setToast(`Імпортовано: ${parts.join(', ')}`);
  };
  ...
  const handleCloseTab = useCallback((tabId: string) => {
    if (tabId === pipTabId) setPipTabId(null);
    setTabs(prev => {
  ...
  const handleExpandPip = useCallback(() => {
    if (pipTabId) setActiveTabId(pipTabId);
    setPipTabId(null);
  }, [pipTabId]);

  const handleActivatePip = useCallback(() => {
    if (!activeTabId || activeTab?.url.startsWith('web20://')) return;
    if (pipTabId === activeTabId) { setPipTabId(null); return; }
    setPipTabId(activeTabId);
    const otherTab = tabs.find(t => t.id !== activeTabId);
    if (otherTab) setActiveTabId(otherTab.id);
  }, [activeTabId, activeTab, pipTabId, tabs]);
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'p': {
          e.preventDefault();
          if (e.shiftKey) handleActivatePip();
          else handlePrint();
          break;
        }
    ...
    <div className={outerCls}>
      <div id="w2-stage" className={stageCls}>
        ...
        <TopBar
          ...
          onActivatePip={isElectron ? handleActivatePip : undefined}
          isPipActive={pipTabId !== null}
        />
        </div>
      ...
      {pipTabId && isElectron && (() => {
        const pipTab = tabs.find(t => t.id === pipTabId);
        const el     = webViewRef.current?.getWebviewEl(pipTabId) ?? null;
        if (!pipTab || !el) return null;
        return (
          <PipWindow
            tabId={pipTabId}
            tabTitle={pipTab.title}
            tabFavicon={pipTab.favicon}
            webviewEl={el}
