    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-[1px]">
          ...
          {onActivatePip && (
            <Tip label={isPipActive ? 'Exit Picture-in-Picture' : 'Picture-in-Picture  Ctrl+Shift+P'} delay={400}>
              <button
      ...
      {bookmarks.length > 0 && (
        <div
          className="flex items-center gap-1 px-3 pb-1.5 overflow-x-auto scrollbar-none"
          style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}
        >
          {bookmarks.map(bm => (
            <div
              key={bm.id}
              className="group flex items-center gap-1.5 px-2 py-[3px] rounded-md shrink-0
                text-white/55 hover:text-white/85 hover:bg-white/[0.06]
                transition-all duration-150 max-w-[160px]"
              title={bm.url}
            >
              <button
              ...
              >
                {bm.favicon
                  ? <img src={bm.favicon} className="w-3 h-3 shrink-0 rounded-sm" alt="" />
                  : <Bookmark size={10} strokeWidth={1.75} className="shrink-0" />
                }
                <span className="text-[11px] truncate leading-none">{bm.title || bm.url}</span>
              </button>
              <button
                ...
                className="opacity-0 group-hover:opacity-100 flex items-center justify-center
                  shrink-0 w-3 h-3 rounded-sm text-white/30 hover:text-white/80
                  transition-all duration-150 cursor-pointer"
