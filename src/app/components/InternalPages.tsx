...
import {
  ...
  Palette, Cookie, Globe, Settings, RotateCcw, Pencil, ChevronRight,
} from "lucide-react";
...
function SettingsPage({
  onClearHistory, onClearBookmarks, onClearDownloads,
  onChooseDownloadPath, onManageCookies, onToggleTheme,
  profiles = [], activeProfileId = 'default',
  onSwitchProfile, onProfilesChange,
}: InternalPagesProps) {
  ...
  const [isDark, setIsDark] = useState(() => {
    try { return localStorage.getItem('w2_theme') !== 'graphite'; } catch { return true; }
  });
  const [downloadPath, setDownloadPath] = useState(() => {
    try { return localStorage.getItem('w2_downloadPath') || 'Downloads'; } catch { return 'Downloads'; }
  });

  const setEngine  = (v: string) => { setSearchEngine(v); try { localStorage.setItem('w2_searchEngine', v); } catch {} };
  const setPrivacy = (v: 'off' | 'balanced' | 'strict') => { setPrivacyMode(v); try { localStorage.setItem('w2_privacy', v); } catch {} };

  const isElectron = !!window.electronAPI?.isElectron;

  const handleClearData = () => {
    (window.electronAPI as any)?.clearSessionData?.();
    onClearHistory(); onClearBookmarks(); onClearDownloads();
    setToast('Дані перегляду та cookies видалено');
  };

  const handleCookies = () => {
    if (onManageCookies) { onManageCookies(); return; }
    setToast('Управління cookies — незабаром');
  };

  const handleTheme = () => {
    const next = isDark ? 'graphite' : 'dark';
    setIsDark(!isDark);
    try { localStorage.setItem('w2_theme', next); } catch {}
    onToggleTheme?.();
    setToast(`Тема змінена: ${next === 'dark' ? 'Dark' : 'Graphite'}`);
  };

  const handleDownloadPath = async () => {
    if (onChooseDownloadPath) { onChooseDownloadPath(); return; }
    if (isElectron) {
      const p = await (window.electronAPI as any)?.chooseDownloadPath?.();
      if (p) {
        setDownloadPath(p);
        try { localStorage.setItem('w2_downloadPath', p); } catch {}
        setToast(`Шлях збереження: ${p}`);
      }
    } else {
      setToast('Вибір шляху доступний у Electron');
    }
  };
  ...
  const handleDeleteProfile = (id: string) => {
    ...
    if (activeProfileId === id) { saveActiveProfileId('default'); onSwitchProfile?.('default'); }
  };

  const NAV_ITEMS: Array<{ id: string; label: string; Icon: React.ComponentType<any> }> = [
    { id: 'profiles',   label: 'Профілі',          Icon: User      },
    ...
    { id: 'languages',  label: 'Мови',              Icon: Globe     },
    { id: 'extensions', label: 'Розширення',        Icon: Puzzle    },
    { id: 'system',     label: 'Система',           Icon: Settings  },
    { id: 'reset',      label: 'Скинути',           Icon: RotateCcw },
  ];
  ...
  const profileOptionRows: Array<{ label: string; action: () => void }> = [
    { label: 'Синхронізувати',        action: () => {} },
    { label: 'Імпорт даних браузера', action: () => setImportOpen(true) },
    { label: 'Параметри профілю',     action: () => {} },
  ];
  ...
  return (
    ...
    >
      {/* ── Sidebar ──────────────────────────────────────────────── */}
      <div className="w-[240px] bg-[#0F0F10] border-r border-white/[0.06] flex flex-col shrink-0 overflow-y-auto">
        <div className="px-3 pt-5 pb-2 shrink-0">
          <div className="flex items-center gap-2 h-8 px-3 rounded-lg bg-white/[0.04] border border-white/[0.07]">
            <Search size={11} strokeWidth={1.75} className="text-white/30 shrink-0" />
            <input
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              placeholder="Пошук налаштувань"
              className="flex-1 bg-transparent outline-none text-[12px] text-white/70 placeholder:text-white/28"
            />
          </div>
        </div>
        <nav className="flex-1 pb-4">
          {filteredNav.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 py-2.5 pr-4 text-[13px]
                transition-all duration-150 cursor-pointer
                ${activeSection === item.id
                  ? 'border-l-2 border-white/60 bg-white/[0.04] text-white/90 pl-[14px]'
                  : 'border-l-2 border-transparent text-white/50 hover:text-white/80 hover:bg-white/[0.04] pl-4'
                }`}
            >
      ...
      <div className="flex-1 overflow-y-auto px-8 py-7 space-y-8">
        ...
        {/* Action buttons */}
        <div>
          <div className="text-[10px] tracking-[0.22em] uppercase text-white/30 mb-3">Основні параметри</div>
          <div className="flex flex-wrap gap-2">
            <ActionButton Icon={Trash2}  label="Видалити дані перегляду" onClick={handleClearData} />
            <ActionButton Icon={Cookie}  label="Управління cookies"       onClick={handleCookies} />
            <ActionButton
              Icon={Palette}
              label={`Тема: ${isDark ? 'Dark' : 'Graphite'}`}
              onClick={handleTheme}
            />
            <div className="flex flex-col">
              <ActionButton Icon={FolderOpen} label="Шлях збереження" onClick={handleDownloadPath} />
              <div className="text-[10px] text-white/30 mt-1 truncate max-w-[200px] px-1">
                {downloadPath}
              </div>
            </div>
          </div>
        ...
        {/* Profiles */}
        <div>
          <div className="text-[10px] tracking-[0.22em] uppercase text-white/30 mb-3">Профілі</div>
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            {profiles.map(p => {
              ...
              return (
                <div key={p.id}
                  className="group flex items-center gap-3 px-4 py-3 border-b border-white/[0.04] last:border-0">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0"
                    style={{ background: p.color + '22', border: `1.5px solid ${p.color}55` }}>
                    <User size={15} strokeWidth={1.75} style={{ color: p.color }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] text-white/80">{p.name}</div>
                    <div className="text-[11px] text-white/30 mt-0.5">{p.partition || 'Default session'}</div>
                  </div>
                  {isActive && (
                    <div className="h-5 px-2 rounded-full bg-white/[0.07] border border-white/[0.1]
                      text-[10px] text-white/55 flex items-center shrink-0">Active</div>
                  )}
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button className="w-7 h-7 rounded-md flex items-center justify-center
                      text-white/30 hover:text-white/70 hover:bg-white/[0.06] transition-all cursor-pointer">
                      <Pencil size={12} strokeWidth={1.75} />
                    </button>
                    {!isBuiltIn && (
                      <button onClick={() => handleDeleteProfile(p.id)}
                        className="w-7 h-7 rounded-md flex items-center justify-center
                          text-white/25 hover:text-red-400/80 hover:bg-red-500/[0.08] transition-all cursor-pointer">
                        <Trash2 size={12} strokeWidth={1.75} />
                      </button>
                    )}
                  </div>
        ...
        {/* Profile option rows */}
        <div>
          <div className="text-[10px] tracking-[0.22em] uppercase text-white/30 mb-3">Параметри профілю</div>
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            {profileOptionRows.map((row, i) => (
              <button key={i} onClick={row.action}
                className="w-full flex items-center justify-between border-b border-white/[0.05] last:border-0
                  py-4 px-5 hover:bg-white/[0.03] transition-colors text-left cursor-pointer">
        ...
        {/* Search engine */}
        <div>
          <div className="text-[10px] tracking-[0.22em] uppercase text-white/30 mb-3">Пошук</div>
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 gap-4">
              <div>
                <div className="text-[13px] text-white/75">Пошукова система</div>
                <div className="text-[11px] text-white/35 mt-0.5">Використовується у рядку адреси</div>
              </div>
              <div className="flex items-center gap-1">
                {(['google', 'duckduckgo', 'bing'] as const).map(engine => (
                  <button key={engine} onClick={() => setEngine(engine)}
                    className={`h-7 px-3 rounded-md text-[11.5px] transition-all duration-150
                      ${searchEngine === engine
                        ? 'bg-white/[0.1] text-white/90 border border-white/[0.12]'
                        : 'text-white/45 hover:text-white/70 hover:bg-white/[0.05]'
                      }`}>
                    {engine === 'duckduckgo' ? 'DDG' : engine.charAt(0).toUpperCase() + engine.slice(1)}
                  </button>
                ))}
              </div>
            </div>
        ...
        {/* Privacy */}
        <div>
          <div className="text-[10px] tracking-[0.22em] uppercase text-white/30 mb-3">Конфіденційність</div>
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 gap-4">
              <div>
                <div className="text-[13px] text-white/75">Захист від стеження</div>
                <div className="text-[11px] text-white/35 mt-0.5">Рівень блокування трекерів</div>
              </div>
              <div className="flex items-center gap-1">
                {(['off', 'balanced', 'strict'] as const).map(mode => (
                  <button key={mode} onClick={() => setPrivacy(mode)}
                    className={`h-7 px-3 rounded-md text-[11.5px] transition-all duration-150
                      ${privacyMode === mode
                        ? 'bg-white/[0.1] text-white/90 border border-white/[0.12]'
                        : 'text-white/45 hover:text-white/70 hover:bg-white/[0.05]'
                      }`}>
                    {mode === 'off' ? 'Вимк' : mode === 'balanced' ? 'Збалансований' : 'Суворий'}
                  </button>
                ))}
              </div>
            </div>
        ...
        {/* About */}
        <div>
          <div className="text-[10px] tracking-[0.22em] uppercase text-white/30 mb-3">Про браузер</div>
          <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
      ...
      {importOpen && <ImportModal onClose={() => setImportOpen(false)} onToast={setToast} />}

      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3
          bg-[#1A1A1C] border border-white/[0.1] rounded-xl px-4 py-2.5
          text-[12px] text-white/80 shadow-[0_8px_24px_rgba(0,0,0,0.5)] pointer-events-auto">
          {toast}
          <button onClick={() => setToast(null)} className="text-white/35 hover:text-white/70 transition-colors">
  ...
  return (
    <button
      ...
      className="flex items-center gap-2.5 px-4 py-3 rounded-xl
        border border-white/[0.1] bg-white/[0.03] hover:bg-white/[0.07]
        text-white/65 hover:text-white/90 transition-all duration-150 cursor-pointer"
    >
      <Icon size={14} strokeWidth={1.75} className="shrink-0 text-white/50" />
...
function ImportModal({ onClose, onToast }: { onClose: () => void; onToast: (msg: string) => void }) {
  const [step,    setStep]    = useState<1 | 2>(1);
  const [source,  setSource]  = useState('edge');
  const [items,   setItems]   = useState<Record<string, boolean>>({
    bookmarks: true, history: true, passwords: false, cookies: false, settings: false,
  });
  ...
  const SOURCES = [
    ...
    { id: 'file',   label: 'З файлу (JSON/HTML)' },
  ];
  ...
  const selectedItems = Object.entries(items).filter(([, v]) => v).map(([k]) => k);
  ...
  const runImport = async () => {
    ...
    try {
      let bmCount = 0;
      if (source === 'file') {
        const filePath = await (window.electronAPI as any)?.openFilePicker?.({
          filters: [{ name: 'Browser data', extensions: ['json', 'html'] }],
        });
        if (!filePath) { setRunning(false); return; }
        const raw = await (window.electronAPI as any)?.readFile?.(filePath);
        if (raw) {
          try {
            const parsed = JSON.parse(raw);
            const bms: any[] = Array.isArray(parsed.bookmarks) ? parsed.bookmarks : [];
            bms.forEach(b => { if (b.url) { addBookmark({ url: b.url, title: b.name || b.title || b.url }); bmCount++; } });
          } catch {}
        }
      } else {
        const imported = await (window.electronAPI as any)?.importBrowserData?.({ browser: source, items: selectedItems });
        if (imported?.bookmarks) {
          (imported.bookmarks as any[]).forEach(b => {
            if (b.url) { addBookmark({ url: b.url, title: b.name || b.title || b.url }); bmCount++; }
          });
        }
      }
      onToast(`Імпорт завершено: ${bmCount} закладок`);
      onClose();
    } catch {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9500]">
      <div className="bg-[#0F0F10] border border-white/[0.1] rounded-2xl
        shadow-[0_24px_64px_rgba(0,0,0,0.7)] w-full max-w-[480px] mx-4 overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
          <div>
            <div className="text-[14px] text-white/88">Імпорт даних браузера</div>
            <div className="text-[11px] text-white/35 mt-0.5">Крок {step} з 2</div>
          </div>
          <button onClick={onClose}
            className="w-7 h-7 rounded-md flex items-center justify-center
              text-white/40 hover:text-white/80 hover:bg-white/[0.07] transition-all cursor-pointer">
            <X size={13} strokeWidth={2} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-2">
          {step === 1 ? (
            <>
              <div className="text-[12px] text-white/40 mb-3">Оберіть джерело</div>
              {SOURCES.map(s => (
                <button key={s.id} onClick={() => setSource(s.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border
                    ...
                    ${source === s.id
                      ? 'border-white/[0.2] bg-white/[0.06] text-white/90'
                      : 'border-white/[0.07] bg-white/[0.02] text-white/55 hover:border-white/[0.12] hover:bg-white/[0.04]'
                    }`}>
                  <div className={`w-3 h-3 rounded-full border-2 shrink-0 transition-all
                    ${source === s.id ? 'border-white/80 bg-white/50' : 'border-white/25'}`} />
            ...
            <>
              <div className="text-[12px] text-white/40 mb-3">Що імпортувати</div>
              {ITEMS.map(item => (
                <button key={item.id}
                  onClick={() => setItems(prev => ({ ...prev, [item.id]: !prev[item.id] }))}
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-white/[0.07]
                    bg-white/[0.02] hover:bg-white/[0.04] transition-all text-left cursor-pointer">
                  <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all
                    ${items[item.id] ? 'border-white/50 bg-white/[0.08]' : 'border-white/20'}`}>
                    {items[item.id] && <div className="w-2 h-1.5 border-l-2 border-b-2 border-white/80 -rotate-45 mt-0.5" />}
                  </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/[0.06]">
          <button onClick={() => step === 1 ? onClose() : setStep(1)}
            className="h-8 px-4 rounded-lg text-[12.5px] text-white/45 hover:text-white/70 transition-colors cursor-pointer">
            {step === 1 ? 'Скасувати' : '← Назад'}
          </button>
          {step === 1 ? (
            <button onClick={() => setStep(2)}
              className="h-8 px-5 rounded-lg text-[12.5px] bg-white/[0.08] text-white/80
                hover:bg-white/[0.12] border border-white/[0.1] transition-all cursor-pointer">
              Далі →
            </button>
          ) : (
            <button onClick={runImport} disabled={running || selectedItems.length === 0}
              className="h-8 px-5 rounded-lg text-[12.5px] bg-white/[0.08] text-white/80
                hover:bg-white/[0.12] border border-white/[0.1] transition-all cursor-pointer
                disabled:opacity-40 disabled:cursor-not-allowed">
              {running ? 'Імпортую…' : 'Імпортувати'}
            </button>
          )}
        </div>
      </div>
