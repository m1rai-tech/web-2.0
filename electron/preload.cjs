...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Clear session cookies + storage (settings page "Clear browsing data")
  clearSessionData:   ()         => ipcRenderer.invoke('session:clear'),
  // Import browser data (Edge / Chrome — full: bookmarks + history + passwords)
  importBrowserData:  (opts)     => ipcRenderer.invoke('browser:import', opts),
  ...
  // Clear browsing cache + storage data
  clearBrowsingData:  ()         => ipcRenderer.invoke('clear-browsing-data'),
});

// Image hover overlay bridge
window.addEventListener('message', (e) => {
  if (!e.data) return;

  if (e.data.__w2reqWcId) {
    const wcId = ipcRenderer.sendSync('webview:get-wc-id');
    e.source?.postMessage({ __w2wcId: wcId }, '*');
    return;
  }

  if (e.data.__w2imgAction) {
    const { __w2imgAction: action, payload, wcId } = e.data;
    ipcRenderer.send('context-menu:action', { action, wcId, payload: payload ?? {} });
  }
});
